...
class Login extends ConsultsBD
{
    ...
    public function registerUser($register)
    {
        try {
            if (isset($_POST['usuario'])) {
                ...
                $create_bd = $this->consult->getRow(TABLE_PASS, '*', array('id_usuario', $register->user));
    ...
    public function createSections($user)
    {
        ...
        $this->consult->insertInto(TABLE_PERSONAL, 'id', $user->id);
        $this->consult->updateRow(TABLE_PERSONAL, 'id_usuario', $user->id_usuario, array('id', $user->id));
        $this->consult->updateRow(TABLE_PERSONAL, 'email', $user->email, array('id', $user->id));
        $this->consult->updateRow(TABLE_PERSONAL, 'fecha_registro', $user->fecha_registro, array('id', $user->id));
