  ...
  this.go = function (r, p, o) {
    ...
    for (var q = 0; q < e.length; q++) {
      ...
      var v = g(l[r])
        ...
        .css(t)
        .appendTo(n);
      v.from = { left: !o ? -v.width() : v.width() - n.position().left };
    ...
    safeOuterLength: function (l) {
      ...
      if (!d.support.csstransforms && d.browser.msie) {
        ...
        if (q && (i == "padding-box" || i == "content-box")) {
          j +=
            parseFloat(d.css(h, "padding-" + o[l][0], true)) ||
            0 + parseFloat(d.css(h, "padding-" + o[l][1], true)) ||
            0;
        }
        if (q && i == "content-box") {
          j +=
            parseFloat(d.css(h, "border-" + o[l][0] + "-width", true)) ||
            0 + parseFloat(d.css(h, "border-" + o[l][1] + "-width", true)) ||
            0;
        }
        this[p] = j;
        return j;
    ...
    if (!f.isEmptyObject(p)) {
      ...
      f.each(p, function (s, u) {
        if (
          j[s] ||
          f.cssAngle[s] ||
          (!f.cssNumber[s] && f.inArray(s, f.transform.funcs) !== -1)
        ) {
          var t = null;
  ...
  function e(u, v, w) {
    ...
    if (
      ...
      (!f.cssNumber[this.prop] &&
        f.inArray(this.prop, f.transform.funcs) !== -1)
    ) {
      this.values = [];
      if (!y) {
        y = 1;
  ...
  f.each(f.transform.funcs, function (k, l) {
    function j(p) {
      var o = p.elem.transform || new f.transform(p.elem),
        n = {};
      if (
        f.cssMultipleValues[l] ||
        (!f.cssNumber[l] && f.inArray(l, f.transform.funcs) !== -1)
      ) {
      ...
      function j(r) {
        ...
        if (!r.initialized) {
          ...
          r.decomposed.start = f.matrix
            ...
            .decompose();
          r.decomposed.end = f.matrix
            .matrix(n[0].end, n[1].end, n[2].end, n[3].end, n[4].end, n[5].end)
            .decompose();
        }
...
function ws_flip(c, n, e) {
  ...
  for (var h = 0; h < m * l; h++) {
    ...
    f(r)
      .css({ position: "absolute", overflow: "hidden" })
      .append(f("<img>").css({ position: "absolute", top: 0, left: 0 }))
      .appendTo(k);
    p(o[0], r, 2 * s + t);
  ...
  function d(w, j, i) {
    if (!c.support.transform) {
      w.each(function (x, y) {
        y = f(y);
        y.animate(
          { width: y.data("width") * 0.8 + "px", height: 0 },
          { easing: "easeInOutCubic", duration: j, complete: i }
        );
      });
    } else {
      w.animate(
  ...
  function b(w, j, i) {
    if (!c.support.transform) {
      w.each(function (x, y) {
        y = f(y);
        y.animate(
          { width: y.data("width") + "px", height: y.data("height") + "px" },
          { easing: "easeInOutCubic", duration: j, complete: i }
        );
      });
    } else {
...
jQuery("#wowslider-container2").wowSlider({
  effect:
    "tv,lines,glass_parallax,parallax,brick,collage,seven,kenburns,cube,blur,book,rotate,domino,slices,blast,blinds,basic,basic_linear,fade,fly,flip,page,stack,stack_vertical",
  prev: "",
  next: "",
  duration: 1000,
  delay:4000,
